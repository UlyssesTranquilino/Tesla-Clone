...
const OrderCar = (props) => {
  ...
  const [paintButtons, setPaintButtons] = useState(currentModel.colors.map(color => color.isActive));
  const [activeWheel, setActiveWheel] = useState(currentModel.wheel.map(wheel => wheel.isActive));
  const [interiorColor, setInteriorColor] = useState(currentModel.Interior.map(interior => interior.isActive));
  ...
  const [isAccessoriesChecked, setIsAccessoriesChecked] = useState(accessories.map(item => item.isActive));


  const backgroundEffectColors = [
    {
      color: "Ultra Red",
      bakground: "radial-gradient(circle at center top, rgb(255, 39, 39) 5%, rgb(0, 0, 0) 90%)",
      background2: "linear-gradient(0deg, rgba(0,0,0,1) 0%, rgba(124,0,0,1) 64%, rgba(43,0,0,1) 86%, rgba(0,0,0,1) 100%)"
    },
    {
      color: "Deep Blue Metallic",
      background: "radial-gradient(circle at center top, rgb(0, 38, 255) 5%, rgb(0, 0, 0) 90%)",
      background2: "linear-gradient(0deg, rgba(0,0,0,1) 0%, rgba(0, 38, 255) 64%, rgba(0,0,43,1) 86%, rgba(0,0,0,1) 100%)"
    },
    {
      color: "Pearl White Multi-Coat",
      background: "radial-gradient(circle at center top, rgb(255, 255, 255) 5%, rgb(0, 0, 0) 90%)",
      background2: "linear-gradient(0deg, rgba(0,0,0,1) 0%, rgb(235, 235, 235) 24%, rgb(77, 77, 77) 76%, rgba(0,0,0,1) 100%)"
    },
    {
      color: "Solid Black",
      background: "radial-gradient(circle at center top, rgb(24, 24, 24) 5%, rgb(0, 0, 0) 90%)",
      background2: "linear-gradient(0deg, rgba(0,0,0,1) 0%, rgb(48, 48, 48) 24%, rgb(5, 5, 5) 76%, rgba(0,0,0,1) 100%)"
    },
    {
      color: "Lunar Silver",
      background: "radial-gradient(circle at center top, rgb(73, 73, 73) 5%, rgb(0, 0, 0) 90%)",
      background2: "linear-gradient(0deg, rgba(0,0,0,1) 0%, rgb(129, 129, 129) 24%, rgb(77, 77, 77) 76%, rgba(0,0,0,1) 100%)"
    }, 
    {
      color: "Stealth Gray",
      background: "radial-gradient(circle at center top, rgb(48, 48, 48) 5%, rgb(0, 0, 0) 90%)",
      background2: "linear-gradient(0deg, rgba(0,0,0,1) 0%, rgb(48, 48, 48) 24%, rgb(27, 27, 27) 76%, rgba(0,0,0,1) 100%)"
    }
  ]

  const [backgroundEffect, setBackgroundEffect] = useState(backgroundEffectColors[0].background)
  const [sideBackgroundEffect, setSideBackgroundEffect] = useState(backgroundEffectColors[0].background2)
  ...
  const buttonToggle = (id) => {
    const selectedCard = dataButton[index].buttons.find(card => card.data === id);
    if (selectedCard) {
  ...
  const paintToggle = (carTitle, index) => {
    setPaintButtons(paintButtons.map((item, num) => num === index ? true : false));
      ...
      <div className='container-fluid order-container-page'>
        <div className='row'>
          <div className='col'>
            <div className='order-image-card front-view'>
              <img src={currentWheel.frontView} className='order-img' alt='Car front view' />
              <div className='order-image-car-bg' style={{background: backgroundEffect}}></div>
            </div>
          </div>
        </div>
            ...
            <div className='car-speed text-center'>
              <div className='row'>
                <div className='col range m-2'>
                  <p>{currentWheel.range.match(/\d+/)[0]}<span>mi</span></p>
                  <p>Range (EPA est.)</p>
                </div>
                <div className='col top-speed m-2'>
                  <p>{topSpeed}<span>mph</span></p>
                  <p>Top Speed</p>
                </div>
                <div className='col sixty-mph m-2'>
                  <p>{time}<span>sec</span></p>
                  <p>0-60mph</p>
                </div>
              </div>
            </div>
        ...
        <div className='row'>
          <div className='col'>
            <div className='buttons-model text-center'>
              <button
                className={`Cash button-type ${showCard.data === 1 ? 'active' : ''}`}
                onClick={() => buttonToggle(1)}
              >
                Cash
              </button>
              <button
                className={`Lease button-type ${showCard.data === 2 ? 'active' : ''}`}
                onClick={() => buttonToggle(2)}
              >
                Lease
              </button>
              <button
                className={`Finance button-type ${showCard.data === 3 ? 'active' : ''}`}
            ...
            <div className='button-card-content'>
              <div className='button-desc d-flex justify-content-between'>
                <p className='showCard-desc'>
                  {showCard.desc}<span className='text-decoration-underline ml-1'>  Customize</span>
                </p>
              ...
              <div className='button-card-buttons'>
                {allButtons}
              </div>
        ...
        <div className='carPaint row'>
          <div className='col car-sideView'>
            <img src={currentWheel.sideView} className='sideView' alt='Car side view' />
          </div>
          <div className='car-sideView-bg' style={{background: sideBackgroundEffect}}>
          ...
          <div className='col paint-section'>
            ...
            <div className='paint-buttons'>
              {currentModel.colors.map((color, index) => (
                <button key={index}>
                  <img
                    src={paintButtons[index] ? color.buttonActive : color.button}
                    alt={`Paint ${index}`}
        ...
        <div className='row carWheel'>
          ...
          <div className='col car-Wheel' style={{background: sideBackgroundEffect}}>
            <img src={currentWheel.wheelView} className='wheelView' alt='Car Wheel View' />
            ...
            <div className='wheel-buttons'>
              <button onClick={() => wheelToggle('WheelOne', 0)}>
                <img src={activeWheel[0] ? currentModel.wheel[0].active
                : currentModel.wheel[0].inActive} alt="Photon Wheels" />
              </button>

              <button onClick={() => wheelToggle('WheelTwo', 1)}>
                <img src={activeWheel[1] ? currentModel.wheel[1].active
                : currentModel.wheel[1].inActive} alt="Nova Wheels" />
              </button>
            </div>
        ...
        {currentModel.TowHitch === '' ? '' : (
          <div className='row tow-hitch'>
            <div className='col'>
              ...
              <div className='col tow-image' style={{background: sideBackgroundEffect}}>
              
                <img src={currentWheel.towView} className='towView' alt='Car Tow View' />
              </div>
        ...
        <div className='row interior'>
          <div className='col'>
            <div className='interior-img'>
              <img src={interiorColor[0] ? currentModel.Interior[0].Black :
              ''} />

              <img src={interiorColor[1] ? currentModel.Interior[1].White :
              ''} />

              <img src={interiorColor[2] ? currentModel.Interior[2].Cream :
              ''} />
            </div>
            ...
            <div className='intorior-desc'>
              ...
              <div className='interior-buttons'>
                <button onClick={() => interiorToggle(0)}>
                  <img src={interiorColor[0] ? currentModel.Interior[0].active : 
                  currentModel.Interior[0].inActive} />
                </button>
                <button onClick={() => interiorToggle(1)}>
                <img src={interiorColor[1] ? currentModel.Interior[1].active : 
                  currentModel.Interior[1].inActive} />
                </button>

                {currentModel.Interior.length > 2 ? 
                  <button onClick={() => interiorToggle(2)}>
                <img src={interiorColor[2] ? currentModel.Interior[2].active : 
              ...
              <div className='interior-desc'>
                <p className='title'>{interiorColor[0] ? currentModel.Interior[0].title : ''}</p>
                <p>{interiorColor[0] ? currentModel.Interior[0].decor : ''}</p>
                <p>{interiorColor[0] ? currentModel.Interior[0].price : ''}</p>

                <p className='title'>{interiorColor[1] ? currentModel.Interior[1].title : ''}</p>
                <p>{interiorColor[1] ? currentModel.Interior[1].decor : ''}</p>
                <p>{interiorColor[1] ? currentModel.Interior[1].price : ''}</p>

                <p className='title'>{interiorColor[2] ? currentModel.Interior[2].title : ''}</p>
                <p>{interiorColor[2] ? currentModel.Interior[2].decor : ''}</p>
                <p>{interiorColor[2] ? currentModel.Interior[2].price : ''}</p>
              </div>
            </div>
        ...
        <div className='row'>
          <div className='col self-driving'>
            <video ref={videoRef} autoPlay control loop>
              <source src={currentModel.selfDriving.video} type="video/webm"/>
            </video>
            ...
            <div className='self-driving-desc'>
                <div className='self-driving-title'>
                  ...
                  <p>Receive a 30 day trial of Full Self-Driving Capability with a new vehicle purchase.</p>
        ...
        <div className='row'>
          ...
          <div className='order-now-title'>
            ...
            <p className='est-delivery'>{currentModel.orderNow.estDelivery}</p>

            {currentModel.orderNow.qualifiesDesc != '' ? 
            <div className='qualifiesDesc'>
              <svg className="icon-large" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 256 256">
                <g fill="#14f238" fillRule="nonzero" stroke="none" strokeWidth="1" strokeLinecap="butt" strokeLinejoin="miter" strokeMiterlimit="10" style={{ mixBlendMode: "normal" }}>
              ...
              <p>{currentModel.orderNow.qualifiesDesc} <span>See Details</span></p>
            </div>
            : ''
            }
  ...
  const largeDevice = () => {
    ...
    <div className="carousel-inner" style={{background: sideBackgroundEffect}}>
      <div className="carousel-item active">
        <img className="d-block w-100 order-img" src={currentWheel.frontView} alt="Front View"/>
      </div>

      <div className="carousel-item">
        <img className="d-block w-100 wheelView" src={currentWheel.wheelView} alt="Wheel View"/>
      </div>

      <div className="carousel-item">
        <div className="interior-img">
          {interiorColor[0] && (
            <img className="d-block w-100" src={currentModel.Interior[0].Black} alt="Interior Black"/>
          )}
          {interiorColor[1] && (
            <img className="d-block w-100" src={currentModel.Interior[1].White} alt="Interior White"/>
          )}
          {interiorColor[2] && (
            <img className="d-block w-100" src={currentModel.Interior[2].Cream} alt="Interior Cream"/>
          )}
        </div>
      </div>
    </div>
  ...
  <a className="carousel-control-next" href="#carouselExampleIndicators" role="button" data-slide="next">
    ...
    <div className="carousel-inner" style={{background: sideBackgroundEffect}}>
      ...
      <div className="carousel-item">
        <img className="d-block w-100 towView" src={currentWheel.towView} alt="Tow View"/>
      </div>
  ...
  return (
    <section className='order-section-view'>
